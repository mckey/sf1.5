...
class sfFilesystem
{
     ...
     * @param boolean $silent Is to log actions
     */
    public function touch($files, $silent = false)
    {
        if (!is_array($files)) {
            $files = [$files];
        }
     ...
     * @param boolean $silent Is to log actions
     */
    public function remove($files, $silent = false)
    {
        if (!is_array($files)) {
            $files = [$files];
        }
     ...
     */
    public function chmod($files, $mode, $umask = 0000, $silent = false)
    {
        $currentUmask = umask();
        umask($umask);
