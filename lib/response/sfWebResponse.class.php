...
class sfWebResponse extends sfResponse
{
    ...
    public static function getDate($timestamp, $type = 'rfc1123')
    {
        ...
        if ($type == 'rfc1123') {
            ...
            throw new InvalidArgumentException(
                'The second getDate() method parameter must be one of: rfc1123, rfc1036 or asctime.'
            );
        }
    }
    ...
    {
        foreach ($this->getPositions() as $position) {
            $this->javascripts[$position] = array_merge(
                $this->getJavascripts($position),
                $response->getJavascripts($position)
            );
            $this->stylesheets[$position] = array_merge(
                $this->getStylesheets($position),
    ...
    protected function fixContentType($contentType)
    {
        ...
        if (false === stripos($contentType, 'charset') && (0 === stripos(
                    $contentType,
                    'text/'
